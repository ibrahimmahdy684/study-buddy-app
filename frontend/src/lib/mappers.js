export const toStudyModeEnum = (ui) => ({
  Online: 'ONLINE',
  'In-person': 'IN_PERSON',
  Both: 'BOTH',
}[ui] ?? 'BOTH');

export const fromStudyModeEnum = (val) => ({
  ONLINE: 'Online',
  IN_PERSON: 'In-person',
  BOTH: 'Both',
}[val] ?? 'Both');

export const toGroupSizeInt = (label) => ({
  '1 (solo)': 1,
  '2-3 people': 2,
  '4-6 people': 4,
  '7+ people': 7,
...
export const fromGroupSizeInt = (val) => {
...
export const toSessionDateTime = (date, time) =>
  new Date(`${date}T${time}`).toISOString();

export const toDurationMinutes = (human) => {
  const map = {
...
export const fromDurationMinutes = (mins) => {
...
export const toUIMessage = (msg, currentUserId) => ({
  id: msg.id,
  from: msg.senderId,
